...
def test_runtime_loads_only_active_completed_snapshot(tmp_path):
    chunks = [
        Chunk(
            ...
            "v",
            0,
            "Genie concepts evidence",
            (),
            "https://docs.databricks.com/x",
            "Docs",
        )
    ]
...
def test_snapshot_manifest_records_the_corpus_fingerprint(tmp_path):
    chunks = [
        Chunk(
            "id",
            "d",
    ...
    build_and_activate(chunks, HashEmbeddingProvider(), tmp_path, corpus_fingerprint="corpus-v1")
    assert read_active_fingerprint(tmp_path) == "corpus-v1"
...
def test_runtime_exposes_chunk_and_document_tools(tmp_path):
    ...
    assert {
        item.chunk.chunk_id
        for item in runtime.search_within_document("https://docs.databricks.com/x", "privilege", 3)
    } == {"first", "second"}

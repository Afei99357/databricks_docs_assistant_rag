...
class HashEmbeddingProvider:
    ...
    def embed(self, texts: Sequence[str]) -> list[list[float]]:
        return [
            [
                (byte - 127.5) / 127.5
                for byte in hashlib.sha256(text.encode()).digest()[: self.dimension]
            ]
            for text in texts
        ]
...
class OllamaEmbeddingProvider:
    ...
    def __init__(
        ...
        self.model_name, self.base_url, self.timeout, self.batch_size, self.dimension = (
            model_name,
            base_url.rstrip("/"),
            timeout,
            batch_size,
    ...
    def embed(self, texts: Sequence[str]) -> list[list[float]]:
        ...
        for start in range(0, len(texts), self.batch_size):
            ...
            print(
                ...
                f"({min(self.batch_size, len(texts) - start)} chunks)...",
                flush=True,
            )
...
class DatabricksEmbeddingProvider:
    ...
    def __init__(
        ...
        *,
        ...
        self.model_name, self.profile, self.batch_size, self.dimension = (
            endpoint,
            profile,
            batch_size,
            0,
    ...
    def embed(self, texts: Sequence[str]) -> list[list[float]]:
        ...
        for batch_number, start in enumerate(batches, start=1):
            ...
            print(
                f"Embedding batch {batch_number}/{total_batches} ({len(batch)} chunks)...",
                flush=True,
            )
            for attempt in range(self.max_rate_limit_retries + 1):
                try:
                    ...
                    if (
                        "REQUEST_LIMIT_EXCEEDED" not in str(exc)
        ...
        if len(values) != len(texts):
            raise RuntimeError(
                "Databricks embedding endpoint returned an unexpected number of vectors"
            )
        if values:
            self.dimension = len(values[0])
        return values

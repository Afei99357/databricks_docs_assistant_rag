...
def load_current_chunks(store: DatabricksStore, chunk_table: str) -> list[Chunk]:
    ...
    chunks = [
        Chunk(
            chunk_id=str(row[0]), doc_id=str(row[1]), document_version=str(row[2]), position=int(row[3]),
            text=str(row[4]), heading_path=heading_path(row[5]), source_url=str(row[6]), source_title=str(row[7]),
        )
        for row in rows
    ]
    if not chunks:
        raise RuntimeError(f"no chunks found in {chunk_table}; run the source refresh before building an index")
    return chunks


def publish_volume_snapshot(store: DatabricksStore, *, namespace: str, volume_path: str,
                            chunks: list[Chunk], embedder) -> PublishedSnapshot:
    ...
    with tempfile.TemporaryDirectory(prefix="rag-app-snapshot-") as temporary:
        ...
        store.upload(Path(temporary) / "active_snapshot.json", f"{volume_path.rstrip('/')}/active_snapshot.json", overwrite=True)

...
CATEGORIES = frozenset({
    "genie-concepts", "agent-mode", "unstructured-data-volumes",
    "knowledge-store-instructions", "benchmarks-evaluation", "genie-apis",
    "genie-one", "genie-code", "agent-framework", "managed-mcp",
    "multi-agent-apps", "uc-permissions-security", "databricks-apps",
})
...
def discover_genie_core(html: str, *, landing_url: str = GENIE_LANDING_URL) -> list[CuratedDoc]:
    ...
    for anchor in BeautifulSoup(html, "lxml").find_all("a", href=True):
        ...
        if parts.netloc != "docs.databricks.com" or not parts.path.startswith("/aws/en/genie"):
            continue
        if parts.path != "/aws/en/genie" and not parts.path.startswith(("/aws/en/genie/", "/aws/en/genie-agents", "/aws/en/genie-one", "/aws/en/genie-code")):
            continue
        doc_id = compute_doc_id(resolved)
        if doc_id not in seen:
            seen.add(doc_id)
            docs.append(CuratedDoc(resolved, resolved, doc_id, compute_slug(resolved), _genie_category(resolved), "aws", "Automatically discovered from the official Genie landing page", "genie-core"))
    return docs
...
def load_curated_docs(yaml_path: str | Path) -> list[CuratedDoc]:
    ...
    for index, entry in enumerate(raw_entries):
        ...
        if doc_id in seen:
            raise SourcesValidationError(f"duplicate doc_id {doc_id} for {url!r} and {seen[doc_id]!r}")
        seen[doc_id] = url
        docs.append(CuratedDoc(url, canonical, doc_id, compute_slug(canonical), category, entry.get("cloud", "aws"), reason))
    return docs

...
def _history_title(question: str) -> str:
...
class ConversationRepository:
    ...
    def create(self, owner: str, title: str) -> str:
        ...
        self.store.execute(f"INSERT INTO {self.conversations} (conversation_id,owner_user_id,title,status,created_at,updated_at) VALUES ({sql_literal(conversation_id)},{sql_literal(owner)},{sql_literal(_history_title(title))},'active',current_timestamp(),current_timestamp())")
    ...
    def delete(self, owner: str, conversation_id: str) -> bool:
        ...
        result = self.store.execute(
            f"SELECT count(*) FROM {self.conversations} WHERE conversation_id={sql_literal(conversation_id)} "
            f"AND owner_user_id={sql_literal(owner)} AND status='active'"
        )
        if int(result.rows[0][0]) != 1:
            return False
        self.store.execute(
            f"UPDATE {self.conversations} SET status='deleted',updated_at=current_timestamp() "
            f"WHERE conversation_id={sql_literal(conversation_id)} AND owner_user_id={sql_literal(owner)}"
        )
        return True
